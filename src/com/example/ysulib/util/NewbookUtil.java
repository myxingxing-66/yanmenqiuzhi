	...
	public List<NewBook> show(){
		...
		try {
			...
			for (int i = 0; i < listbook.size(); i++) {
				...
				newbook.setName(listbook.get(i).getElementsByTag("a").text()
						.toString());
				newbook.setDetailUrl(listbook.get(i).getElementsByTag("a")
						.attr("href").substring(2, 35).toString());
				newbook.setIsbn(listbook.get(i).getElementsByTag("h3").get(0)
						.childNode(1).toString());
				newbook.setInfo(listbook.get(i).getElementsByTag("p").get(0)
